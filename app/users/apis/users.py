...
def createUser( req:schema.insertUser):
    db= next(db_session())
    try:
        ...
        user=models.Users(**req.dict(exclude_none=True))
        db.add(user)
        db.commit()
...
@routes.post("/login")
def userLogin(req:schema.UserLogin):
    """
    ...
    try:
        ...
        user=user.__dict__
        del user["password"]
        db.query(models.Users).filter(models.Users.email==req.email).update(values={"logged_in":True})
        db.commit()
        return user
    except HTTPException as http_error:
        raise HTTPException(status_code=http_error.status_code, detail=http_error.detail)
...
def resetUserPassword(req:schema.ResetPassword):
    """
    Endpoint to reset user password.
    """
    db=next(db_session())
    try:
        user=db.query(models.Users).filter(models.Users.email==req.email, models.Users.id_number==req.id_number, models.Users.phone_number==req.phone_number)
        if not user.first():
            raise HTTPException(status_code=400, detail="User Not Found.")
        req.password=hash_password(req.password)
...
def getUserDetail(user_id:str):
    ...
    db=next(db_session())
    try:
        user=db.query(models.Users).filter(models.Users.id==user_id).options(defer("password")).first()
        if not user:
            raise HTTPException(status_code=400, detail="User Not Found.")
        return user
    except HTTPException as http_error:
